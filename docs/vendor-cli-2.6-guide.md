...
| CLI 版本 | 维护日期 | 更新内容 |
| --- | --- | --- |
| 2.6.1 | 2026-09-23 | 新增供应商管理员软删除待审核安全产品工时命令，并补充 `work_record_voided` 流程事件说明。 |
| 2.6.0 | 2026-09-20 | 新增交付材料上传、删除，以及更换安全产品订单工程师命令。 |

> 适用对象：云集供应商负责人和经授权的供应商工程师
> 版本定位：包含供应商基础业务命令，以及安全产品工时、流程事件、交付材料写操作和订单成员维护能力
...
| 登录、身份确认、退出 | 支持 | 支持 |
| 员工查询和维护 | 支持 | 不支持 |
| 需求订单查询、普通订单/安全产品订单接单、拒单 | 支持 | 不支持 |
| 安全产品订单工程师更换 | 支持全量替换 | 不支持 |
| 采购单查询和响应 | 支持 | 不支持 |
| 安全产品工时查询、填写、修改 | 支持本供应商工程师 | 支持本人工时 |
| 工时核对结果查询 | 支持采购单和订单 | 不支持；员工查询本人工时记录 |
| 流程事件查询 | 授权范围内 | 不支持 |
| 订单交付材料查询、上传、删除和下载 | 授权范围内 | 本人绑定订单；删除仅限本人上传 |

供应商员工不开放 `work-hours-check`、`process-trace` 和订单工程师更换。员工上传材料时，服务端会校验本人已绑定对应订单；删除材料时只能删除本人上传的记录。
...
```bash
yunji requirement-order-approve-product \
  --id <安全产品需求订单ID> \
  --yes --compact
```

普通订单和安全产品订单的接单接口不同。执行前应先查看订单详情确认订单类型；安全产品订单不得使用普通接单命令。
...
yunji requirement-order-update-engineers \
  --id <安全产品需求订单ID> \
  --user-ids <完整工程师用户ID列表> \
  --yes --compact
```

`userIds` 是全量替换，不是增量追加。执行前先读取订单详情中的 `projectUsers`，把所有需要保留的原成员与新增成员一起传给 `--user-ids`。成功后重新读取订单详情确认结果。
...
```bash
yunji work-hours-delete \
  --id <工时记录ID> \
  --reason '<删除原因>' \
  --yes --compact
```

该命令仅供应商管理员可用，只能软删除本供应商状态为 `2`（待审核）的安全产品工时。删除后会产生 `work_record_voided` 流程事件；供应商员工不开放此命令。

工时必须是正整数小时。日期不能晚于当天，必须落在订单服务时间内；同一订单、工程师和日期不能重复填写。`--user-id` 始终表示实际工作的工程师，不是登录操作人；代填操作人和修改历史由服务端记录。

### 9. 工时核对

工时核对结果供供应商负责人在响应采购单前核对交付与工时。供应商工程师使用 `work-hours-list` 和 `work-hours-detail` 查看本人工时记录即可。
...
流程事件是供应商负责人核对订单流转和留痕的只读能力，供应商工程师不开放。
...
`work_record_voided` 表示供应商管理员软删除了一条待审核工时。该事件用于审计留痕，不应作为首次填写或最后填写工时的业务时间点。
...
```bash
yunji material-upload \
  --requirement-order-id <需求订单ID> \
  --file ./delivery-material.docx \
  --yes --compact
```

可重复传入 `--file` 一次绑定多个文件。CLI 会先检查文件存在、非空、不超过 100M、扩展名在平台白名单内，并拒绝重复文件名。上传成功后不会输出对象存储地址；请使用 `materials-by-order` 确认绑定结果。接口不做服务端重复校验，同一文件不要重复提交。

软删除材料：

```bash
yunji material-delete \
  --id <材料ID> \
  --yes --compact
```

删除是软删除，历史记录保留但默认查询不再返回。供应商管理员可删除本供应商订单材料；供应商员工只能删除本人上传的材料。
